...
class ConditionCategory:
    """
        分工况建模
    """
    def __init__(self, wheel_speed_threshold=8, wind_turbine_current_state=6, hydraulic_brake_pressure_threshold=150,
                 gen_power_threshold=2000, power_threshold=2000, gmm_n_components=3, gmm_random_state=0,
        ...
        if col_name_map is None:
            self.col_name_map = dict(time_col="记录时间", wind_speed="机舱气象站风速",
                                     ...
                                     turbine_state="风机当前状态值", hydraulic_brake_pressure="液压制动压力",
                                     gen_power_threshold="发电机功率限幅值", power_threshold="风机功率限制设定值")
        else:
        ...
        self.hydraulic_brake_pressure_threshold = hydraulic_brake_pressure_threshold  # 液压制动压力阈值
    ...
    def mechanism_filter(self, df):
        ...
        data = df[(df[self.col_name_map['turbine_state']] ==self.wind_turbine_current_state)].copy()

        # 剩余过滤条件视风场字段情况
        if self.col_name_map['wheel_speed']:
            data = data[data[self.col_name_map['wheel_speed']] >= self.wheel_speed_threshold].copy()
        if self.col_name_map['hydraulic_brake_pressure']:
            data = data[data[self.col_name_map['hydraulic_brake_pressure']]
                        >= self.hydraulic_brake_pressure_threshold].copy()
    ...
    @staticmethod
    def drop_inf_and_nan(inf_and_nan_df):
        """去除极值、空值

        Parameters
        ----------
        inf_and_nan_df:DataFrame
            可能有空值或极值的数据
        ...
        """
        inf_and_nan_df.replace([np.inf, -np.inf], np.nan, inplace=True)
        inf_and_nan_df.dropna(axis=0, how="any", inplace=True)
        return inf_and_nan_df
    ...
    def condition_category_result(self, handled_df, labels, wind_turbine):
        ...
        """
        feature_cols = list(self.col_name_map.values())[1:4]
        handled_df["label"] = labels
        res_df = handled_df.groupby("label").mean()
