...
def get_data_name_list(base_path):
    ...
    data_list : list
        返回指定文件夹下文件夹列表
    """
    file_list = list(base_path.glob("*.csv"))
...
def data_pre_processing(original_df):
    ...
    lof_df = condition.algorithm_denoising(filter_df)
    return lof_df
...
def condition_category_result_split(sorted_df, data_list_reset, gmm_plot_picture_path):
    ...
    # 按工况划分分类结果
    condition1_res = list(category_df.iloc[0])
    condition2_res = list(category_df.iloc[1])
    condition3_res = list(category_df.iloc[2])
    # 按字段划分分类结果[[w,wh,p],[],[]]
...
def dataset_split(lof_df, condition_category_threshold_list):
    ...
    condition1_df = model.condition_split(
        lof_df, condition_category_threshold_list[0][0], condition_category_threshold_list[0][1],
        condition_category_threshold_list[0][2], condition_category_threshold_list[1][0],
        condition_category_threshold_list[1][1], condition_category_threshold_list[1][2])
    condition2_df = model.condition_split(
        lof_df, condition_category_threshold_list[1][0], condition_category_threshold_list[1][1],
        condition_category_threshold_list[1][2], condition_category_threshold_list[2][0],
        condition_category_threshold_list[2][1], condition_category_threshold_list[2][2])
    condition3_df = model.condition_split(
        lof_df, condition_category_threshold_list[2][0], condition_category_threshold_list[2][1],
...
def model_construct(condition_df, frequency, condition_krr_pre_result_plot_save_path, wind_turbine, num):
    ...
    model_construct_result_list : list
        分工况预测建模结果集合列表
    """
    model_construct_result_list = model.conditon_model_construct(condition_df, frequency)
...
def main():
    ...
    for wind_turbine in data_list:
        ...
        # 保存分类结果并存入csv
        result_df = condition.condition_category_result(
            handled_df, labels, wind_turbine)
    ...
    for col in data_col:
        sorted_df = all_condition.sort_category_result(data, col, sorted_df)
    sorted_df.columns = data_col
        ...
        for index, condition_df in enumerate(condition_df_list):
            if condition_df.empty:
                ...
                model_construct_result_list \
                    = model_construct(condition_df, "50s", condition_krr_pre_result_plot_path, wind_turbine, "2")
