...
class PltAll:
    ...
    def gmm_scatter(self, handled_df, labels, wind_turbine_id, save_path):
        ...
        ax.scatter(handled_df[self.category_list[0]],
                   handled_df[self.category_list[1]], s=15, c=labels)
    ...
    def category_plot(split_df, plt_name, save_path, wind_turbine_id):
        ...
        plt.fill_between(xf, mean1, mean3 + (mean3 - mean2),
                         color="#B0C4DE", alpha=0.25)
        plt.legend(loc="best")
        plt.xlabel("风机")
        plt.ylabel(plt_name)
        plt.savefig(str(save_path.joinpath(wind_turbine_id, "_gmm3D.png")), transparent=True)
