...
class AllDatasetConditionCategoryMean:
    ...
    def __init__(self, category_str="2D_GMM", col_name_map=None):
        if col_name_map is None:
            self.col_name_map = dict(time_col="记录时间", wind_speed="机舱气象站风速",
                                     wheel_speed="轮毂转速", power="变频器发电机侧功率", tempe_col="主轴承温度1")
        else:
    ...
    def category_result_mean_sum(self, sorted_df, wind_turbine_id_list):
        ...
        wind_tmp_df = split_wind_df.to_numpy().mean(axis=0)
        wheel_tmp_df = split_wheel_df.to_numpy().mean(axis=0)
        power_tmp_df = split_power_df.to_numpy().mean(axis=0)
        category_df = pd.DataFrame(np.vstack((wind_tmp_df, wheel_tmp_df, power_tmp_df)))
        ...
        # 输出split_wind_df, split_wheel_df, split_power_df画图分析需要
